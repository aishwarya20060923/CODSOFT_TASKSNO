...
export default function StudentFeesClient({ invoices }: { invoices: FeeInvoiceItem[] }) {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-slate-900 tracking-tight">Fee Invoices & Receipts</h1>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="bg-white p-4 rounded-xl border border-slate-200/80 shadow-2xs">
          <span className="text-xs text-slate-500 font-semibold uppercase">Total Semester Fees</span>
        ...
        <div className="bg-white p-4 rounded-xl border border-emerald-100 bg-emerald-50/20 shadow-2xs">
          <span className="text-xs text-emerald-700 font-semibold uppercase">Cleared Payments</span>
          <p className="text-2xl font-bold text-emerald-800 mt-1">{formatCurrency(totalPaid)}</p>
          <p className="text-xs text-emerald-600 mt-0.5">Verified by Bursar</p>
        </div>

        <div className="bg-white p-4 rounded-xl border border-amber-100 bg-amber-50/20 shadow-2xs">
          <span className="text-xs text-amber-700 font-semibold uppercase">Pending Dues</span>
      ...
      <div className="bg-white rounded-2xl border border-slate-200/80 shadow-xs overflow-hidden">
        <div className="p-4 border-b border-slate-100 flex items-center justify-between text-xs text-slate-500">
          <span className="font-semibold text-slate-700">Official Fee Schedule</span>
          ...
          {invoices.map((inv) => (
            ...
            >
              <div className="flex items-start gap-4">
                ...
                <div>
                  <h3 className="font-bold text-slate-900 text-base">{inv.title}</h3>
                  <div className="flex flex-wrap items-center gap-2 mt-1 text-xs text-slate-500">
                    <span className="font-mono text-slate-600 font-medium">{inv.invoiceNumber}</span>
                    <span>•</span>
              ...
              <div className="flex items-center justify-between sm:justify-end gap-4 shrink-0">
                <div className="text-right">
                  <p className="font-bold text-slate-900 text-lg">{formatCurrency(inv.amount)}</p>
                  <span
                ...
                {inv.status !== 'PAID' && (
                  <button
                    ...
                    className="px-4 py-2 text-xs font-semibold bg-purple-600 hover:bg-purple-700 text-white rounded-xl shadow-xs transition"
                  >
                    Pay Online Now
                  </button>
                )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-xs">
          <div className="bg-white rounded-2xl max-w-md w-full p-6 shadow-2xl border border-slate-100 animate-in fade-in zoom-in-95">
            ...
            <div className="mt-4 p-4 rounded-xl bg-slate-50 border border-slate-200/80">
              <span className="text-xs text-slate-500 uppercase font-semibold">Paying For</span>
              <p className="font-bold text-slate-900 mt-0.5">{selectedInvoice.title}</p>
              <div className="flex items-baseline justify-between mt-2 pt-2 border-t border-slate-200">
                <span className="text-xs text-slate-600">Total Due Amount</span>
            ...
            <div className="mt-6 flex items-center justify-end gap-2 pt-3 border-t border-slate-100">
              ...
              <button
                ...
                onClick={handlePay}
                disabled={processing}
                className="inline-flex items-center gap-1.5 px-4 py-2 text-xs font-semibold bg-purple-600 hover:bg-purple-700 text-white rounded-lg transition disabled:opacity-50"
              >
